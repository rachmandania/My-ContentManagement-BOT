...
if command == "start":
    ...
    success_message = (
        f"✅ **Video Generation Complete!**\n\n"
        f"🎮 **Selected Game:** {game_name}\n"
        f"🎥 Both Horizontal (10m) and Vertical (2m 50s) native clips are rendered and ready for upload."
    )
    payload = {"content": success_message}
    requests.post(discord_webhook, json=payload)
    print("Successfully sent success notification to Discord!")
    ...
    upload_msg = (
        ...
        f"📺 **Uploaded Video:** `[Fan Content] {game_name}`\n"
        f"🏷️ All 3 game tags attached & AI Synthetic Media flag declared."
    )
    requests.post(discord_webhook, json={"content": upload_msg})
    print("Upload notification sent.")
